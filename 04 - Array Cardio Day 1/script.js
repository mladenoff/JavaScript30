...
const inventorsByYear = inventors.slice(0).sort((a, b) =>
  a.year - b.year,
);
...
const totalInventorLifespans = inventors.reduce(
  (accum, inventor) => accum + inventorLifespan(inventor),
  0,
);

console.log(totalInventorLifespans);

// 5. Sort the inventors by years lived

const inventorsByLifespan = inventors.slice(0).sort((a, b) =>
  inventorLifespan(a) - inventorLifespan(b),
);
...
const isSubString = function isSubString(string, subString) {
  for (let idx = 0; idx < string.length - subString.length; idx += 1) {
    if (string.slice(idx, idx + subString.length) === subString) { return true; }
  }
  return false;
};

const boulevardsDeDe = boulevardArray.filter(street => isSubString(street, 'de'));

console.table(boulevardsDeDe);

// 7. sort Exercise
// Sort the people alphabetically by last name
const getFirstName = name => name.split(', ')[1];

// IM: I have sorted by first name because, unless I am missing something,
// sorting by last name here is trivial (it is just sorting the strings)

const peopleByFirstName = people.slice(0).sort((a, b) =>
  (getFirstName(a) < getFirstName(b) ? -1 : 1),
);

// const peopleByLastname = people.slice(0).sort();
...
const vehicleCounts = data.reduce((accum, vehicle) => {
  if (accum[vehicle] === undefined) {
    accum[vehicle] = 0;
  }
  accum[vehicle] += 1;
  return accum;
